...
export async function generateSitemaps() {
  try {
    ...
    const sitemapsNeeded = Math.max(1, Math.ceil(allRecipes.length / SITEMAP_SIZE))
    
    console.log(`Generating ${sitemapsNeeded} sitemaps for ${allRecipes.length} recipes`)
...
export default async function sitemap({
  ...
  try {
    ...
    console.log(`Generated ${recipesForThisSitemap.length} URLs for sitemap ${id}`)
    
    return recipesForThisSitemap.map((recipe: Recipe) => ({
      url: `https://zavecheria.com/recipe/${recipe.documentId}/${formatNameForUrl(recipe.title)}`,
